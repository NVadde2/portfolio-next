...
This wasn't just an individual habit — we worked on it as a team, continually refining our shared instructions and building a framework around it: writing code, reviewing code, committing, reviewing PRs, using shared skills, raising PRs — AI woven into the full lifecycle, not just autocomplete in the editor. One person's fix to a bad pattern became everyone's fix, instead of five engineers rediscovering the same quirk separately.
...
That "what vs. why" gap is exactly why [I've come to care so much about guardrails at the architecture level](/blog/hexagonal-architecture-nextjs) — typed boundaries, validated data contracts, an error hierarchy that doesn't let a malformed assumption drift silently through the app. AI-generated code and API responses have more in common than people think: neither should be trusted blind, and the fix for both is the same. 

The AI was never doing the hard part. The guardrails were. The impressive-seeming result (working, reliable software) was never actually produced by the AI acting alone/magically. The hard part — the thing that actually made the output trustworthy — was always the human-built structure around it.
